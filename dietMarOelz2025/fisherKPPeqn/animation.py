...
for i in range(N):
	fig,ax = plt.subplots()
	ax.plot(x,u[0,:],'k-',label='Density of cells initially')
	ax.plot(x,u[s*i,:],'r-',label='Density of cells at present moment')
	ax.set_xlim(-2,2)
	ax.set_ylim(-0.25,1.5)
	ax.legend(loc='upper left',fontsize=10)
	ax.set_xlabel("Position (in cm)",fontsize=10)
	ax.set_ylabel("Density of cell in the tumor",fontsize=10)
	plt.tight_layout()
